...
const NAV_ITEMS: NavItem[] = [
  ...
  { path: '/history', label: 'Tarefas', icon: History },
]
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-0.5">
        ...
        <div>
          ...
          {!collapsed && isSettings && (
            <div className="ml-[21px] mt-0.5 space-y-px border-l border-[var(--border)] pl-3">
              {[
                { label: 'Geral', hash: 'general' },
                { label: 'Estratégia de Registro', hash: 'register' },
                { label: 'Serviço de E-mail', hash: 'mailbox' },
                { label: 'Captcha', hash: 'captcha' },
                { label: 'SMS', hash: 'sms' },
                { label: 'Proxies', hash: 'proxies' },
                { label: 'ChatGPT', hash: 'chatgpt' },
                { label: 'Avançado', hash: 'advanced' },
      ...
      <div className="shrink-0 border-t border-[var(--border)] px-2 py-1.5 flex items-center gap-1">
        <button
          ...
          title={theme === 'light' ? 'Mudar para escuro' : theme === 'dark' ? 'Mudar para claro' : 'Seguir sistema'}
        >
  ...
  return (
    <div className="flex h-screen items-center justify-center bg-[var(--bg-base)]">
      <form onSubmit={submit} className="w-80 space-y-4 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-6">
        ...
        <p className="text-sm text-[var(--text-muted)]">Digite a senha de acesso</p>
        <input
          type="password"
          value={pw}
...
export default function App() {
  ...
  if (authState === 'loading') {
    return <div className="flex h-screen items-center justify-center bg-[var(--bg-base)] text-[var(--text-muted)] text-sm">Carregando...</div>
  }
  if (authState === 'locked') {
